...
if __name__ == "__main__":
    ...
    ENV = VisionOAAviary(gui=GUI, record=RECORD, obstacles=True)
    ...
    TARGET_POSITION = np.array([[0, 1.0*np.cos(0.006*i), 1.0] for i in range(DURATION*ENV.SIM_FREQ)])
    # TARGET_POSITION = np.array([[0.0, -0.5, 1.0] for i in range(DURATION*ENV.SIM_FREQ)])
